...
public class BombController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if(timeToExplode <= 0)
        {
            ...
            Collider2D[] obejectsToRemove = Physics2D.OverlapCircleAll(transform.position, blastRange, whatIsDestructible);

            if(obejectsToRemove.Length > 0)
            {
                foreach(Collider2D col in obejectsToRemove)
                {
